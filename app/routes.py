...
def get_git_user_info():
    ...
    try:
        ...
        name = name_result.stdout.strip() if name_result.returncode == 0 else 'Unknown User'
...
def update_mitigation_link():
    ...
    try:
        ...
        if 'mitigation_links' in dhf_data and link_id in dhf_data['mitigation_links']:
            ...
            return jsonify({'success': True, 'message': 'Mitigation link updated successfully'})
        else:
...
def update_configuration():
    ...
    try:
        ...
        if action == 'add':
            ...
            return jsonify({'success': True, 'message': f'Added new {config_type} option', 'new_id': new_id})

        elif action == 'remove':
            option_id = data.get('option_id')
            if not option_id:
                return jsonify({'error': 'Option ID is required for remove action'}), 400
            ...
            if not option_id or not name:
                return jsonify({'error': 'Option ID and name are required for update action'}), 400
            result = data_manager.update_config_option(config_type, option_id, name, description)

        else:
            return jsonify({'error': 'Invalid action'}), 400

        if result:
            return jsonify({'success': True, 'message': f'Configuration updated successfully'})
        else:
