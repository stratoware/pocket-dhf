...
class DHFDataManager:
    ...
    def load_data(self) -> Dict[str, Any]:
        ...
        if self._data is None:
            try:
                ...
                raise FileNotFoundError(f"DHF data file not found: {self.data_file_path}")
    ...
    def get_risks_flat(self) -> Dict[str, Any]:
        ...
        if not any(isinstance(group, dict) and 'risks' in group for group in risks_data.values()):
            return risks_data
    ...
    def update_item(self, item_id: str, updated_item: Dict[str, Any]) -> bool:
        ...
        for group_key, group in data.get('product_requirements', {}).items():
            if 'requirements' in group and item_id in group['requirements']:
                data['product_requirements'][group_key]['requirements'][item_id].update(updated_item)
                self.save_data(data)
                return True
        ...
        for group_key, group in data.get('software_specifications', {}).items():
            if 'specifications' in group and item_id in group['specifications']:
                data['software_specifications'][group_key]['specifications'][item_id].update(updated_item)
                self.save_data(data)
                return True
        ...
        for group_key, group in data.get('hardware_specifications', {}).items():
            if 'specifications' in group and item_id in group['specifications']:
                data['hardware_specifications'][group_key]['specifications'][item_id].update(updated_item)
                self.save_data(data)
                return True
    ...
    def update_folder_name(self, group_type: str, group_key: str, new_name: str) -> bool:
    ...
    def get_configuration(self) -> Dict[str, Any]:
        ...
        probability_occurrence_mapping = config.get('probability_occurrence_mapping', {})
        probability_harm_mapping = config.get('probability_harm_mapping', {})

        # Default mappings if none found
        if not severity_mapping:
            severity_mapping = {
                'S1': {'name': 'Low', 'description': 'Minor impact, low risk to patient safety'},
                'S2': {'name': 'Medium', 'description': 'Moderate impact, potential for patient harm'},
                'S3': {'name': 'High', 'description': 'Significant impact, serious risk to patient safety'}
            }

        if not probability_mapping:
            probability_mapping = {
                'P1': {'name': 'Low', 'description': 'Unlikely to occur under normal conditions'},
                'P2': {'name': 'Medium', 'description': 'May occur occasionally during normal use'},
                'P3': {'name': 'High', 'description': 'Likely to occur frequently during normal use'}
            }

        if not probability_occurrence_mapping:
            probability_occurrence_mapping = {
                'PO1': {'name': 'Low', 'description': 'Unlikely to occur under normal conditions'},
                'PO2': {'name': 'Medium', 'description': 'May occur occasionally during normal use'},
                'PO3': {'name': 'High', 'description': 'Likely to occur frequently during normal use'}
            }

        if not probability_harm_mapping:
            probability_harm_mapping = {
                'PH1': {'name': 'Low', 'description': 'Unlikely to cause harm if it occurs'},
                'PH2': {'name': 'Medium', 'description': 'May cause harm if it occurs'},
                'PH3': {'name': 'High', 'description': 'Likely to cause harm if it occurs'}
            }
        ...
        return {
            ...
            'probability_occurrence_ids_in_use': list(probability_occurrence_ids_in_use),
            'probability_harm_ids_in_use': list(probability_harm_ids_in_use)
        }

    def add_config_option(self, config_type: str, name: str, description: str = '') -> str:
        ...
        existing_ids = [k for k in data['configuration'][mapping_key].keys() if k.startswith(prefix)]
    ...
    def remove_config_option(self, config_type: str, option_id: str) -> bool:
        ...
        if ('configuration' in data and
            mapping_key in data['configuration'] and
            option_id in data['configuration'][mapping_key]):
    ...
    def update_config_option(self, config_type: str, option_id: str, name: str, description: str = '') -> bool:
        ...
        if ('configuration' in data and
            ...
            if description:
                data['configuration'][mapping_key][option_id]['description'] = description
    ...
    def get_probability_name(self, probability_id: str) -> str:
        ...
        config = self.get_configuration()
        return config['probability_mapping'].get(probability_id, {}).get('name', probability_id)
    ...
    def get_probability_occurrence_name(self, probability_occurrence_id: str) -> str:
        ...
        config = self.get_configuration()
        return config['probability_occurrence_mapping'].get(probability_occurrence_id, {}).get('name', probability_occurrence_id)

    def get_probability_harm_name(self, probability_harm_id: str) -> str:
        """Get the display name for a probability harm ID."""
        config = self.get_configuration()
        return config['probability_harm_mapping'].get(probability_harm_id, {}).get('name', probability_harm_id)

    def calculate_rbm_score(self, probability_occurrence_id: str, probability_harm_id: str, severity_id: str) -> int:
        """Calculate RBM score: Probability of Occurrence × Probability of Harm × Severity."""
        # Map IDs to numeric values (1, 2, 3)
        po_value = int(probability_occurrence_id.replace('PO', '')) if probability_occurrence_id.startswith('PO') else 1
        ph_value = int(probability_harm_id.replace('PH', '')) if probability_harm_id.startswith('PH') else 1
        s_value = int(severity_id.replace('S', '')) if severity_id.startswith('S') else 1
