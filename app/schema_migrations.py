...
def migrate_v1_to_v2(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (
        ...
        and "probability_occurrence_mapping" in data["configuration"]
    ):
            ...
            if isinstance(group_data, dict) and "risks" in group_data:
                ...
                for risk_id, risk in group_data["risks"].items():
                    if "probability_occurrence" in risk:
                        logger.debug(
                            f"Removing probability_occurrence from risk {risk_id}"
                        )
                        del risk["probability_occurrence"]
                        risks_migrated += 1
            elif (
                isinstance(group_data, dict) and "probability_occurrence" in group_data
            ):
...
def migrate_to_latest(data: Dict[str, Any], from_version: str = None) -> Dict[str, Any]:
    ...
    if from_version == CURRENT_SCHEMA_VERSION:
        logger.debug(
            f"Data is already at current schema version {CURRENT_SCHEMA_VERSION}"
        )
        return data
